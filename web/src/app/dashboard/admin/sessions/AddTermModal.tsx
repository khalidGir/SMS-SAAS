...
export default function AddTermModal({ sessionId, onClose, onSuccess }: AddTermModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/v1/academic/terms', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ academicSessionId: sessionId, name, startDate, endDate }),
      });
      const json = await res.json();
      if (res.ok) { onSuccess(); } else { setError(json.error?.message || 'Failed to create term'); }
    } catch { setError('Network error'); }
    finally { setSaving(false); }
  };

  return (
    <div className={`fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm pt-24 ${ANIMATIONS.fadeIn}`}>
      <div className={`mx-4 w-full max-w-md rounded-lg bg-white shadow-xl ${ANIMATIONS.scaleIn}`}>
        <div className="flex items-center justify-between border-b px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Add Term</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6">
          ...
          <div className="mt-6 flex justify-end gap-3 border-t pt-4">
            ...
            >{saving ? 'Saving...' : 'Create Term'}</button>
          </div>
        </form>
      </div>
    </div>
