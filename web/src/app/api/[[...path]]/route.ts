...
function createResponse(resolve: (res: NextResponse) => void) {
  let statusCode = 200;
  ...
  function resolveResponse() {
    ...
    resolve(new NextResponse(body, { status: statusCode, statusText: statusMessage, headers: resHeaders }));
  }
  ...
  return {
    ...
    json(body: any) { setHeader('content-type', 'application/json; charset=utf-8'); chunks.push(Buffer.from(JSON.stringify(body))); },
    status(code: number) { statusCode = code; return this; },
    sendStatus(code: number) { statusCode = code; resolveResponse(); },
    send(body: any) {
    ...
    append(field: string, val: string | string[]) {
      ...
      headers[k] = existing ? [...(Array.isArray(existing) ? existing : [existing]), ...(Array.isArray(val) ? val : [val])] : (Array.isArray(val) ? val : [val]);
    },
    format() {}, vary() {}, links() {},
  };
}
...
async function handleWithExpress(nextReq: NextRequest): Promise<NextResponse> {
  ...
  return new Promise((resolve) => app(req, createResponse(resolve)));
}
